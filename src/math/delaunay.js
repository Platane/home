 ...
 *
 */
export const delaunay = (points: Point[]): Array<[number, number, number]> => {
    //// calc a wrapping triangle

    // calc the bounding box
    let box = boundingBox(points);
    box = expandBoundingBox(
        box,
        Math.max(box.max.x - box.min.x, box.max.y - box.min.y) * 5
    );
    ...
    points.slice(0, -3).forEach((A, i) => {
        ...
        const concerned = circles.reduce(
            (arr, c, i) => (squareDistance(c, A) < c.r && arr.push(i), arr),
            []
        );

        // build the convex hull with this
        const hullEdges = concerned
            .reduce((arr, i) => {
                // push the all 3 edges of the triangle

                // grab the 3 edges
                const edges = triangles[i].map((a, i, arr) => {
                    let b = arr[(i + 1) % 3];
            ...
            .filter(
                (edge, i, arr) =>
                    !arr.some(
                        (edge_, j) =>
                            i != j && edge[0] == edge_[0] && edge[1] == edge_[1]
                    )
            );

        // remove the triangles
        concerned.reverse().forEach(i => {
    ...
    triangles = triangles.filter(triangle =>
        triangle.every(i => i < points.length)
    );
