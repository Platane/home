...
const quantumizedColor = Array.from({ length: 3 }).map(
    (_, i, arr) => `hsl(0,0%,${10 + i / arr.length * 5}%)`
)
const paint = (ctx: CanvasRenderingContext2D, l: number): void => {
    // ctx.fillStyle = `hsl(${Math.random()*360},60%,70%)`
    // ctx.fillStyle = '#6f5f5c'
    ctx.fillStyle =
        quantumizedColor[0 | (Math.random() * quantumizedColor.length)]
...
let img
loadImage(
    'https://media.licdn.com/media/AAEAAQAAAAAAAAoJAAAAJDRlZWVhMmNiLTIzZGItNDczYi1iYmI3LTEyN2UzYjg5OGQ5OA.jpg'
).then(_ => {
    img = _
